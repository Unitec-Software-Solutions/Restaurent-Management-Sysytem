        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
            ...
            <div class="p-6 border-b flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                ...
                <div class="flex gap-2">
                    <a href="{{ route('admin.grn.index') }}" class="bg-gray-200 hover:bg-gray-300 text-gray-800 px-4 py-2 rounded-lg flex items-center">
                        <i class="fas fa-arrow-left mr-2"></i> Back to GRNs
                    </a>
                    <a href="{{ route('admin.grn.show', $grn->grn_id) }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg flex items-center">
            ...
            <form action="{{ route('admin.grn.update', $grn->grn_id) }}" method="POST" class="p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select id="branch_id" name="branch_id" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent" required>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select id="supplier_id" name="supplier_id" class="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent" required>
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-sm text-left text-gray-700">
                                <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                                    <tr>
                                        <p class="text-sm font-medium text-gray-700 text-m px-4 py-3 ">Items of {{ $grn->grn_number }}</p>
                                    </tr>
                                </thead>
                                            ...
                                            <td class="px-4 py-3" colspan="9">
                                                ...
                                                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-3">
                                                    <div>
                                                        ...
                                                        <input type="number" step="0.01" name="items[{{ $loop->index }}][ordered_quantity]" value="{{ $item->ordered_quantity }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg ordered-qty" required>
                                                    </div>
                                                    <div>
                                                        <label class="block text-xs font-medium text-gray-500 mb-1">Received Qty *</label>
                                                        <input type="number" step="0.01" name="items[{{ $loop->index }}][received_quantity]" value="{{ $item->received_quantity }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg received-qty" required>
                                                    </div>
                                                    <div>
                                                        <label class="block text-xs font-medium text-gray-500 mb-1">Accepted Qty *</label>
                                                        <input type="number" step="0.01" name="items[{{ $loop->index }}][accepted_quantity]" value="{{ $item->accepted_quantity }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg accepted-qty" required>
                                                    </div>
                                                    <div>
                                                ...
                                                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                                                    <div>
                                                        ...
                                                        <input type="number" step="0.01" name="items[{{ $loop->index }}][buying_price]" value="{{ $item->buying_price }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg price" required>
                                                    </div>
                                                    <div>
                                                        <label class="block text-xs font-medium text-gray-500 mb-1">Line Total</label>
                                                        <div class="w-full px-3 py-2 font-medium line-total">${{ number_format($item->line_total, 2) }}</div>
                                                    </div>
                                                    <div>
                                                        <label class="block text-xs font-medium text-gray-500 mb-1">MFG Date</label>
                                                        <input type="date" name="items[{{ $loop->index }}][manufacturing_date]" value="{{ $item->manufacturing_date ? $item->manufacturing_date->format('Y-m-d') : '' }}" class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                                                    </div>
                                                    <div>
                ...
                <div class="mb-6">
                    ...
                    <textarea id="notes" name="notes" rows="3" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">{{ $grn->notes }}</textarea>
                </div>
