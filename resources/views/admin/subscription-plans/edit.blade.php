    ...
    <form action="{{ route('admin.subscription-plans.update', $subscriptionPlan->id) }}" method="POST" class="space-y-4">
        ...
        <div class="mb-4">
            ...
            <input type="checkbox" name="is_trial" value="1"
                {{ old('is_trial', $subscriptionPlan->is_trial) ? 'checked' : '' }}>
        </div>
        <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700 mb-1">Trial Period (days)</label>
            <input type="number" name="trial_period_days"
                value="{{ old('trial_period_days', $subscriptionPlan->trial_period_days ?? 30) }}"
                min="1" max="365" class="w-full px-4 py-2 border rounded-lg">
        </div>
        <div>
