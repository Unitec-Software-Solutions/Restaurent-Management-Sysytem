...
<div class="p-6">
    ...
    <div class="mb-6">
        <div class="flex items-center gap-4">
            <a href="{{ route('admin.menus.show', $menu) }}" class="text-gray-500 hover:text-gray-700">
                <i class="fas fa-arrow-left text-xl"></i>
            </a>
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Edit Menu: {{ $menu->name }}</h1>
                <p class="text-gray-600">Modify menu details and items</p>
            </div>
        </div>
    </div>
    ...
    <form method="POST" action="{{ route('admin.menus.update', $menu) }}" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="type" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                        ...
                        <option value="breakfast" {{ old('type', $menu->type) === 'breakfast' ? 'selected' : '' }}>Breakfast</option>
                        <option value="lunch" {{ old('type', $menu->type) === 'lunch' ? 'selected' : '' }}>Lunch</option>
                        <option value="dinner" {{ old('type', $menu->type) === 'dinner' ? 'selected' : '' }}>Dinner</option>
                        <option value="all_day" {{ old('type', $menu->type) === 'all_day' ? 'selected' : '' }}>All Day</option>
                        <option value="special" {{ old('type', $menu->type) === 'special' ? 'selected' : '' }}>Special</option>
                        <option value="seasonal" {{ old('type', $menu->type) === 'seasonal' ? 'selected' : '' }}>Seasonal</option>
                    </select>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="3" 
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                              placeholder="Describe this menu...">{{ old('description', $menu->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <select name="branch_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                        ...
                        @foreach($branches as $branch)
                            <option value="{{ $branch->id }}" {{ old('branch_id', $menu->branch_id) == $branch->id ? 'selected' : '' }}>
                                {{ $branch->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="flex items-center">
                    <label class="flex items-center">
                        ...
                        <span class="ml-2 text-sm text-gray-700">Menu is active</span>
                    </label>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="date" name="valid_from" value="{{ old('valid_from', $menu->valid_from?->format('Y-m-d')) }}" required
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                ...
                <div>
                    ...
                    <input type="date" name="valid_until" value="{{ old('valid_until', $menu->valid_until?->format('Y-m-d')) }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                ...
                <div>
                    ...
                    <input type="time" name="start_time" value="{{ old('start_time', $menu->start_time) }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                ...
                <div>
                    ...
                    <input type="time" name="end_time" value="{{ old('end_time', $menu->end_time) }}"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
            ...
            <div class="mt-6">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
                    @php 
                        ...
                        $oldDays = old('available_days', $menu->available_days ?? []);
                        // Ensure $oldDays is always an array
                        if (!is_array($oldDays)) {
                            $oldDays = [];
                        }
                    @endphp
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" id="items-grid">
                @foreach($menuItems as $item)
                    <div class="item-card border border-gray-200 rounded-lg p-4 hover:bg-gray-50 {{ in_array($item->id, $attachedItems) ? 'bg-indigo-50 border-indigo-200' : '' }}" 
                         data-category="{{ $item->category_id }}" data-name="{{ strtolower($item->name) }}">
                        <label class="flex items-start cursor-pointer">
                            <input type="checkbox" name="menu_items[]" value="{{ $item->id }}" 
                                   {{ in_array($item->id, old('menu_items', $attachedItems)) ? 'checked' : '' }}
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            <div class="flex justify-between items-center">
                <div class="text-sm text-gray-500">
                    Last updated: {{ $menu->updated_at ? $menu->updated_at->format('M j, Y g:i A') : 'Not available' }}
                </div>
                <div class="flex gap-3">
                    <a href="{{ route('admin.menus.show', $menu) }}" class="bg-gray-200 hover:bg-gray-300 text-gray-800 px-6 py-2 rounded-lg">
                        Cancel
                    </a>
