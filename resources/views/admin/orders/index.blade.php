        ...
        @if (config('app.debug'))
            <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
                ...
                <div class="text-xs text-yellow-700 mt-2 grid grid-cols-3 gap-4">
                    <div>
                        <p><strong>Orders Variable:</strong>
                            {{ isset($orders) ? 'Set (' . $orders->count() . ')' : 'NOT SET' }}</p>
                        <p><strong>DB Total Orders:</strong> {{ \App\Models\Order::count() }}</p>
                    </div>
                    <div>
                        <p><strong>Today's Orders:</strong>
                            {{ \App\Models\Order::whereDate('created_at', today())->count() }}</p>
                            ...
                            <tr class="border-t hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3">
                                    @if ($order->reservation_id)
                                        <a href="{{ route('admin.reservations.show', $order->reservation_id) }}"
                                            class="text-blue-500">
