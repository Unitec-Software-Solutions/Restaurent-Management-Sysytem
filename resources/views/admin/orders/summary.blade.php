    ...
    @if (config('app.debug'))
        <div class="bg-pink-50 border border-pink-200 rounded-lg p-4 mb-6">
            <div class="flex justify-between items-center">
                ...
                <a href="{{ request()->fullUrlWithQuery(['debug' => 1]) }}" class="text-xs text-pink-600 hover:text-pink-800">
                    Full Debug (debug=1)
                </a>
            </div>
            <div class="text-xs text-pink-700 mt-2 grid grid-cols-3 gap-4">
                ...
                <div>
                    <p><strong>Items with Menu:</strong>
                        {{ isset($order) ? $order->orderItems->filter(fn($i) => !is_null($i->menuItem))->count() : 'N/A' }}
                    </p>
                    <p><strong>Items with Inventory:</strong>
                        {{ isset($order) ? $order->orderItems->filter(fn($i) => !is_null($i->inventoryItem))->count() : 'N/A' }}
                    </p>
                    <p><strong>Missing Items:</strong>
                        {{ isset($order) ? $order->orderItems->filter(fn($i) => is_null($i->menuItem) && is_null($i->inventoryItem))->count() : 'N/A' }}
                    </p>
                </div>
            </div>
        </div>
            ...
            <div class="p-6 md:p-8">
                    ...
                    <div>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex">
                                ...
                                <div>: <span class="px-2 py-1 bg-green-100 text-green-800 rounded text-sm">Ready for
                                        pickup</span></div>
                            </div>
                            <div class="flex">
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($order->items as $item)
                                    <tr class="hover:bg-gray-50">
                                            ...
                                            <div class="flex items-center">
                                                <div class="ml-4">
                                                    <div class="text-sm font-medium text-gray-900">
                                                        {{ $item->menuItem->name }}</div>
                                                    @if ($item->special_requests)
                                                        <div class="text-sm text-gray-500">{{ $item->special_requests }}
                                                        </div>
                                                    @endif
                                                </div>
                                            </div>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">LKR
                                            {{ number_format($item->unit_price, 2) }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">LKR
                    ...
                    <div class="bg-gray-50 p-4 rounded-lg">
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            @if ($order->discount > 0)
                                ...
                                <div class="font-medium text-green-600">- LKR {{ number_format($order->discount, 2) }}
                                </div>
                            @endif
                        </div>
                ...
                <div class="mt-8 flex flex-wrap gap-3 no-print">
                    <a href="{{ route('orders.takeaway.edit', $order->id) }}"
                        class="flex-1 md:flex-none bg-yellow-500 hover:bg-yellow-600 text-white font-medium py-2 px-4 rounded-lg transition duration-200 flex items-center justify-center">
                        <i class="fas fa-edit mr-2"></i> Update Order
                    </a>

                    <form action="{{ route('orders.takeaway.destroy', $order->id) }}" method="POST"
                    ...
                    <button onclick="window.print()"
                        class="flex-1 md:flex-none bg-gray-200 hover:bg-gray-300 text-gray-800 font-medium py-2 px-4 rounded-lg transition duration-200 flex items-center justify-center">
                        <i class="fas fa-print mr-2"></i> Print Receipt
                    </button>
                    <a href="{{ route('admin.orders.takeaway.index') }}"
                        class="flex-1 md:flex-none bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-medium py-2 px-4 rounded-lg transition duration-200 flex items-center justify-center">
