...
<div class="max-w-7xl mx-auto bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
    ...
    <div class="mb-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
            ...
            <div class="bg-yellow-50 dark:bg-yellow-900/30 p-4 rounded-lg border border-yellow-100 dark:border-yellow-900">
                <div class="flex items-center justify-between">
                    <div>
                    ...
                    <div class="p-3 rounded-full bg-yellow-100 dark:bg-yellow-800/50 text-yellow-600 dark:text-yellow-300">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    <div class="mb-6 bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg">
        <form method="GET" action="{{ route('admin.inventory.stock.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <!-- Search -->
            <div>
            ...
            <div>
                ...
                <select name="status" class="w-full rounded-lg border-gray-300 shadow-sm dark:bg-gray-700 dark:text-white dark:border-gray-600 focus:border-blue-500 focus:ring-blue-500">
            ...
            <div class="flex items-end space-x-2">
                <button type="submit" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg flex items-center justify-center w-full">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-1" viewBox="0 0 20 20" fill="currentColor">
                ...
                @if(request()->anyFilled(['search', 'branch_id', 'status']))
                    <a href="{{ route('admin.inventory.stock.index') }}" class="px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white rounded-lg flex items-center justify-center w-full">
                        Clear
                    </a>
                @endif
            </div>
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                            ...
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    ...
                                    <div class="text-sm text-gray-500 dark:text-gray-400">
                                        {{ $stock['item']->item_code }} • {{ optional($stock['item']->category)->name ?? 'Uncategorized' }}
                                    </div>
                                </div>
                            </div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm text-gray-900 dark:text-white">
                                ...
                                <span class="text-xs text-gray-500 dark:text-gray-400 ml-1">{{ $stock['item']->unit_of_measurement }}</span>
                            </div>
                        
                            <div class="w-full bg-gray-200 rounded-full h-1.5 dark:bg-gray-700 mt-1">
                                @php
                                    $percentage = ($stock['reorder_level'] > 0) 
                                        ? min(100, ($stock['current_stock'] / $stock['reorder_level']) * 100) 
                                        : 0;
                            
                                    $color = $stock['status'] === 'low_stock' ? 'bg-yellow-500' : 'bg-green-500';
                                @endphp
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @if($stock['status'] === 'low_stock')
                                <span class="px-2.5 py-0.5 inline-flex items-center text-xs font-medium rounded-full bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-100">
                                    <svg class="-ml-0.5 mr-1.5 h-2 w-2 text-yellow-500 dark:text-yellow-300" fill="currentColor" viewBox="0 0 8 8">
                                        <circle cx="4" cy="4" r="3" />
                                    </svg>
                                    Low Stock
                                </span>
                            @else
                                <span class="px-2.5 py-0.5 inline-flex items-center text-xs font-medium rounded-full bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-100">
                                    <svg class="-ml-0.5 mr-1.5 h-2 w-2 text-green-500 dark:text-green-300" fill="currentColor" viewBox="0 0 8 8">
                                        <circle cx="4" cy="4" r="3" />
                                    </svg>
                                    In Stock
                                </span>
