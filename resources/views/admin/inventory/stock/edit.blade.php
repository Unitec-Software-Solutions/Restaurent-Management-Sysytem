...
<div class="p-4 rounded-lg">
    <div class="max-w-4xl mx-auto bg-white rounded-xl shadow-sm p-6">
        ...
        <form
            action="{{ route('admin.inventory.stock.update', ['item_id' => $transaction->inventory_item_id, 'branch_id' => $transaction->branch_id]) }}"
            ...
            <div>
                ...
                <div class="relative">
                    <select name="inventory_item_id" disabled
                        ...
                        <option value="{{ $item->id }}" data-unit="{{ $item->unit_of_measurement }}" {{ $transaction->
                            inventory_item_id == $item->id ? 'selected' : '' }}>
                ...
                <div class="relative">
                    <select name="transaction_type" required
                        ...
                        <optgroup label="Stock Out">
                            <option value="sales_order" {{ $transaction->transaction_type == 'sales_order' ? 'selected'
                                : '' }}>Sales Order
                            </option>
                            <option value="write_off" {{ $transaction->transaction_type == 'write_off' ? 'selected' : ''
                                }}>Write Off
                            </option>
                            <option value="transfer" {{ $transaction->transaction_type == 'transfer' ? 'selected' : ''
                                }}>Transfer</option>
                            <option value="usage" {{ $transaction->transaction_type == 'usage' ? 'selected' : '' }}>
                                Usage</option>
                            <option value="transfer_out" {{ $transaction->transaction_type == 'transfer_out' ?
            ...
            <div class="flex justify-between items-center pt-6 border-t">
                ...
                <div class="flex space-x-3">
                    <button type="reset" class="bg-gray-100 hover:bg-gray-200 text-gray-700 px-6 py-2.5 rounded-lg">
                        Reset Changes
                    </button>
                    <button type="submit"
