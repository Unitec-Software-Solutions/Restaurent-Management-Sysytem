    ...
    <div class="mx-auto px-4 py-8">
        <div class="max-w-4xl mx-auto bg-white rounded-xl shadow-sm p-6">
            ...
            <div class="mb-6">
                ...
                <p class="text-sm text-gray-500"> Dev ~ edit stock directly ~</p>
            </div>
                        ...
                        <select name="transaction_type" required
                            ...
                            <optgroup label="Stock Out">
                                <option value="sales_order"
                                    {{ $transaction->transaction_type == 'sales_order' ? 'selected' : '' }}>
                                    Sales Order
                                </option>
                                <option value="write_off"
                                    {{ $transaction->transaction_type == 'write_off' ? 'selected' : '' }}>Write Off
                                </option>
                                <option value="transfer"
                                    {{ $transaction->transaction_type == 'transfer' ? 'selected' : '' }}>Transfer</option>
                                <option value="usage" {{ $transaction->transaction_type == 'usage' ? 'selected' : '' }}>
                                    Usage</option>
                                <option value="transfer_out"
                                    {{ $transaction->transaction_type == 'transfer_out' ? 'selected' : '' }}>
                                    Transfer Out
                                </option>
                            </optgroup>
                        </select>
