...
@section('content')
    <div class="p-4 rounded-lg">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            <!-- Item Profile Card -->
            <div
                class="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden col-span-1 md:col-span-2 lg:col-span-1">
                <div class="p-6 flex flex-col items-center">
                    @if ($item->attributes['img'] ?? false)
                        <img src="{{ asset('storage/' . $item->attributes['img']) }}" alt="{{ $item->name }}"
                            class="h-32 w-32 object-cover rounded-lg mb-4 shadow-md">
                    @else
                        <div
                            class="h-32 w-32 bg-indigo-100 dark:bg-indigo-900 rounded-lg flex items-center justify-center mb-4">
                    ...
                    <div class="mt-4 flex flex-wrap justify-center gap-2">
                        <span
                            class="px-3 py-1 bg-indigo-100 dark:bg-indigo-900 text-indigo-800 dark:text-indigo-200 rounded-full text-xs">
                            {{ $item->category->name ?? 'No Category' }}
                        </span>
                        @if ($item->is_perishable)
                            <span
                                class="px-3 py-1 bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200 rounded-full text-xs">
                                Perishable
                            </span>
                        @endif
                        @if ($item->is_menu_item)
                            <span
                                class="px-3 py-1 bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 rounded-full text-xs">
                                Menu Item
                            </span>
                        @endif
                    </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
                ...
                <div class="p-6 space-y-4">
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium text-gray-900 dark:text-white">Rs.
                            {{ number_format($item->buying_price, 2) }}</span>
                    </div>
                    <div class="flex justify-between">
                    ...
                    @if ($item->is_menu_item && isset($item->attributes['discount']))
                        <div class="flex justify-between">
                            ...
                            <span
                                class="font-medium text-indigo-600 dark:text-indigo-400">{{ $item->attributes['discount'] }}%</span>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden md:col-span-2">
                ...
                <div class="p-6">
                    <div class="mb-4">
                        ...
                        <p class="text-gray-700 dark:text-gray-300">{{ $item->description ?? 'No description available' }}
                        </p>
                    </div>
                    <div>
                        <h4 class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">Additional Notes</h4>
                        <p class="text-gray-700 dark:text-gray-300">{{ $item->additional_notes ?? 'No additional notes' }}
                        </p>
                    </div>
                </div>
