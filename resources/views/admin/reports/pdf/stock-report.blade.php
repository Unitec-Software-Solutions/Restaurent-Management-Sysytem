...
@if($viewType === 'detailed' && $reportData->count() > 0)
<div class="page-break"></div>

<div class="mt-4">
    <h3 style="font-size: 14px; font-weight: bold; margin-bottom: 15px; color: #2563eb;">Stock Analysis Summary</h3>

    <!-- Category Breakdown -->
    @php
        $categoryBreakdown = $reportData->groupBy('category.name')->map(function($items, $category) {
            return [
                'category' => $category ?: 'Uncategorized',
                'total_items' => $items->count(),
                'in_stock' => $items->where('stock_status', 'In Stock')->count(),
                'low_stock' => $items->where('stock_status', 'Low Stock')->count(),
                'out_of_stock' => $items->where('stock_status', 'Out of Stock')->count(),
    ...
    <div class="avoid-break">
        <h4 style="font-size: 12px; font-weight: 600; margin-bottom: 10px;">Stock Status by Category</h4>
        <table class="data-table">
            <thead>
                <tr>
                    <th>Category</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="text-center text-success">{{ $category['in_stock'] }}</td>
                    <td class="text-center text-warning">{{ $category['low_stock'] }}</td>
                    <td class="text-center text-danger">{{ $category['out_of_stock'] }}</td>
                    <td class="text-right">Rs. {{ number_format($category['total_value'], 2) }}</td>
                </tr>
                @endforeach
            </tbody>
        </table>
    ...
    @php
        $criticalItems = $reportData->where('stock_status', '!=', 'In Stock');
    @endphp

    @if($criticalItems->count() > 0)
    <div class="avoid-break mt-4">
        <h4 style="font-size: 12px; font-weight: 600; margin-bottom: 10px; color: #dc2626;">Items Requiring Attention</h4>
        <table class="data-table">
            <thead>
                <tr>
                    <th>Item Name</th>
                    ...
                    <th class="text-right">Potential Loss</th>
                </tr>
            </thead>
            <tbody>
                @foreach($criticalItems->take(20) as $item)
                <tr>
                    <td class="font-semibold">{{ $item->name }}</td>
                    <td class="text-sm">{{ $item->category->name ?? 'N/A' }}</td>
                    <td class="text-center">{{ number_format($item->current_stock, 2) }}</td>
                    ...
                    <td class="text-right">
                        @if($item->stock_status === 'Out of Stock' && $item->selling_price)
                            Rs. {{ number_format(($item->reorder_level ?? 0) * $item->selling_price, 2) }}
                        @else
...
<div style="text-align: center; padding: 40px; color: #6b7280;">
    <div style="font-size: 14px; margin-bottom: 10px;">No stock data found</div>
