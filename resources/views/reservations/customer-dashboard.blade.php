...
@section('content')
    <div class="container mx-auto max-w-2xl py-8">
            ...
            <div class="bg-purple-50 border border-purple-200 rounded-lg p-4 mb-6">
                ...
                <div class="text-xs text-purple-700 mt-2 grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <p><strong>Reservations Variable:</strong>
                            {{ isset($reservations) ? 'Set (' . count($reservations) . ')' : 'NOT SET' }}</p>
                    </div>
                    <div>
        ...
        <form method="GET" action="{{ route('customer.dashboard') }}" class="mb-8">
            <div class="flex flex-col md:flex-row items-center gap-4">
                <input type="text" name="phone" value="{{ old('phone', $phone ?? '') }}"
                    placeholder="Enter your phone number" class="border rounded px-4 py-2 w-full md:w-2/3" required>
        ...
        @if (isset($phone) && $phone)
            <h3 class="text-xl font-semibold mb-4">Reservations for <span class="text-blue-700">{{ $phone }}</span>
            </h3>
                        ...
                        <tbody>
                            @foreach ($reservations as $reservation)
                                <tr>
                                    ...
                                    <td class="px-4 py-2 border">{{ $reservation->start_time }} -
