...
export async function activateBranch(page: Page) {
  ...
  await page.screenshot({ path: 'activate-branch-step-1-summary.png' });
  ...
  await page.screenshot({ path: 'activate-branch-step-2-login-filled.png' });
  ...
  await page.screenshot({ path: 'activate-branch-step-3-after-copy.png' });
  ...
  await page.screenshot({ path: 'activate-branch-step-4-activation-page.png' });
  await page.getByRole('textbox').fill(activationKey);
  await page.screenshot({ path: 'activate-branch-step-5-key-filled.png' });
  ...
  try {
    ...
    await page.screenshot({ path: 'activate-branch-step-6-result.png' });
    console.log('Activation result displayed');
  } catch (e) {
    console.warn('No result message appeared after activation');
    await page.screenshot({ path: 'activate-branch-step-6-no-result.png' });
  }
}
